...
const users = [
{ id: 1, username: "user1", password: "password1", role: "read" },
{ id: 2, username: "user2", password: "password2", role: "write" },
];
...
app.get("/read", verifyToken, checkRole("read"), (req, res) => { // Read route
    res.status(200).json({message:"Read operation successful!"});
});

app.post("/write", verifyToken, checkRole("write"), (req, res) => { // Write route
    res.status(200).json({message:"Read operation successful!"});
});
